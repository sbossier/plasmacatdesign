...
# Read in starting data & replace couple of forms of zero with a NA (ugly done, maybe by using regex it could be more elegantly done)
filename <- "res-time-02.5s-1-test.csv"
...
# Note 2: 3.71 is the z-value for which 99.99% of values fall within a normal curve.
...
data <- data %>%
  mutate( flowSD = ( .005 * flow + .001 * flowMax ) / 3.71 ,
          flowRSD = flowSD / flow ,
          co2flowSD = ( .005 * co2flow + .001 * maxco2flow) / 3.71 ,
          co2flowRSD = co2flowSD / co2flow ,
          betaValue = n2flow/co2flow ,
          betaValueRSD = 0 ,
          betaValueSD = 0 )
...
data <- data %>%
  mutate( betaValue2 = concAvgN2Blank / concAvgCO2Blank ,
          betaValue2RSD = 0 ,
          betaValue2SD = 0 )
...
data <- data %>%
  mutate( alphaValue = ifelse(name == "blank", 1, concAvgN2Blank / concAvgN2Plasma * (1 + betaValue) - betaValue) ,
          alphaValueSD = 0 ,
          alphaValueRSD = 0 ,
          alphaValue2 = ifelse(name == "blank", 1, concAvgN2Blank / concAvgN2Plasma * (1 + betaValue2) - betaValue2) )
...
data <- data %>%
  mutate(concAvgCorr = concAvg * (1 + betaValue / alphaValue ) ,
         concAvgCorrRSD = sqrt(
         ...
         concAvgCorrSD = concAvgCorrRSD * concAvgCorr
  )

# This correction however, has no physical meaning for N2, so to avoid confusion, we'll delete is for N2

data <- data %>%
  mutate(concAvgCorr = ifelse(compound == "N2", NA, concAvgCorr) ,
         concAvgCorrSD = ifelse(compound == "N2", NA, concAvgCorrSD) ,
         concAvgCorrRSD = ifelse(compound == "N2", NA, concAvgCorrRSD)
  )

#################################
# Calculating conversion of CO2 #
#################################
...
data <- data %>%
  mutate(convCO2 = ifelse(compound == "CO2" & name == "plasma", convCO2, NA) ,
         convCO2SD = ifelse(compound == "CO2" & name == "plasma", convCO2SD, NA) ,
         convCO2RSD = ifelse(compound == "CO2" & name == "plasma", convCO2RSD, NA))
...
# Calculate gamma
# Obtaining N2 flow (in mLn/min)

n2flow <- data %>%
  pull(n2flow) %>%
  unique()

# Obtaining CO2 flow (in mLn/min) in and its SD & RSD
...
gamma <- co2flow / ( co2flow + n2flow )

gamma2 <- concAvgCO2Blank / ( concAvgCO2Blank + concAvgN2Blank )
...
############################################
# Calculating the Carbon-Selectivity of CO #
############################################
...
data <- data %>%
  mutate(CselecCO = ifelse(compound == "CO" & name == "plasma", CselecCO, NA) ,
         CselecCOSD = ifelse(compound == "CO" & name == "plasma", CselecCOSD, NA) ,
         CselecCORSD = ifelse(compound == "CO" & name == "plasma", CselecCORSD, NA))

######################################
# Calculating the Carbon-Yield of CO #
######################################
...
data <- data %>%
  mutate(CyieldCO = ifelse(compound == "CO" & name == "plasma", CyieldCO, NA) ,
         CyieldCOSD = ifelse(compound == "CO" & name == "plasma", CyieldCOSD, NA) ,
         CyieldCORSD = ifelse(compound == "CO" & name == "plasma", CyieldCORSD, NA))

############################################
# Calculating the Oxygen-Selectivity of CO #
############################################
...
data <- data %>%
  mutate(OselecCO = ifelse(compound == "CO" & name == "plasma", OselecCO, NA) ,
         OselecCOSD = ifelse(compound == "CO" & name == "plasma", OselecCOSD, NA) ,
         OselecCORSD = ifelse(compound == "CO" & name == "plasma", OselecCORSD, NA))

######################################
# Calculating the Oxygen-Yield of CO #
######################################
...
data <- data %>%
  mutate(OyieldCO = ifelse(compound == "CO" & name == "plasma", OyieldCO, NA) ,
         OyieldCOSD = ifelse(compound == "CO" & name == "plasma", OyieldCOSD, NA) ,
         OyieldCORSD = ifelse(compound == "CO" & name == "plasma", OyieldCORSD, NA))

############################################
# Calculating the Oxygen-Selectivity of O2 #
############################################
...
data <- data %>%
  mutate(OselecO2 = ifelse(compound == "O2" & name == "plasma", OselecO2, NA) ,
         OselecO2SD = ifelse(compound == "O2" & name == "plasma", OselecO2SD, NA) ,
         OselecO2RSD = ifelse(compound == "O2" & name == "plasma", OselecO2RSD, NA))
...
data <- data %>%
  mutate(OyieldO2 = ifelse(compound == "O2" & name == "plasma", OyieldO2, NA) ,
         OyieldO2SD = ifelse(compound == "O2" & name == "plasma", OyieldO2SD, NA) ,
         OyieldO2RSD = ifelse(compound == "O2" & name == "plasma", OyieldO2RSD, NA))
...
data <- data %>%
  mutate(yieldRatio = ifelse(compound == "CO" & name == "plasma", yieldRatio, NA) ,
         yieldRatioSD = ifelse(compound == "CO" & name == "plasma", yieldRatioSD, NA) ,
         yieldRatioRSD = ifelse(compound == "CO" & name == "plasma", yieldRatioRSD, NA))
...
data <- data %>%
  mutate(Cbalance = ifelse(compound == "CO" & name == "plasma", Cbalance, NA) ,
         CbalanceSD = ifelse(compound == "CO" & name == "plasma", CbalanceSD, NA) ,
         CbalanceRSD = ifelse(compound == "CO" & name == "plasma", CbalanceRSD, NA))
...
data <- data %>%
  mutate(Obalance = ifelse(compound == "O2" & name == "plasma", Obalance, NA) ,
         ObalanceSD = ifelse(compound == "O2" & name == "plasma", ObalanceSD, NA) ,
         ObalanceRSD = ifelse(compound == "O2" & name == "plasma", ObalanceRSD, NA))
...
data <- data %>%
  group_by(name, compound) %>%
  mutate(powerAvg = mean(power, na.rm = T) ,
         powerSD = sd(power, na.rm = T) ,
         powerRSD = powerSD / powerAvg ,
         powerAvg = ifelse(name == "blank", NA, powerAvg) ,
         powerSD = ifelse(name == "blank", NA, powerSD) ,
         powerRSD = ifelse(name == "blank", NA, powerRSD)) %>%
  ungroup()
...
SEI <- ( powerAvg / co2flow ) * 60 * 24.055

# Calculate SEI RSD

SEIRSD <- sqrt( powerRSD ^ 2 + co2flowRSD ^ 2 )
...
etaSnoeckx <- ( ( GfCO - GfCO2 ) * convCO2 ) / SEI * 100

etaSnoeckxRSD <- sqrt(
  convCO2RSD ^ 2 + SEIRSD ^ 2
)
