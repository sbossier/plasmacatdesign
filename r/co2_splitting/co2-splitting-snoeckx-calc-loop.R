# from supplier-material-xy.zs-01.csv to supplier-material-xy.zs-01-calc.csv

# run over all materials of a supplier in CO2-splitting

# nested loop
# create overview-conv-eq-k-values.csv
# create overview-conversion-co2.csv
# create overview-energy-efficiency.csv
  # for every folder in "H:/data/co2-splitting/uhasselt/*"
    # create "material"-conv-values.csv
    # for every xy.zs-0w.csv calculate xy.zs-0w-calc.csv
    # use median & MAD?
...
filename <- "res-time-70.0s-03.csv"
...
gas_temp_rsd         <- sqrt(gas_temp_in_rsd^2 + gas_temp_out_rsd^2)
...
ee_table$RSD <- c(ee_gf_rsd*100, ee_lhv_rsd*100, ee_hhv_rsd*100)
...
epx_par_table <- tibble(Parameter = c("Flow rate N2 (mL/min)", "Flow rate CO2 (mL/min)", "Gamma", "Delta", "Plasma Power (W)", "SEI (kJ/mol)", "Yield Ratio CO:O2", "# Measurements", "Avg Gas Temp (°C)"))
epx_par_table$Value <- c(flow_n2, flow_co2, gamma, delta, plasma_power_avg, sei, yield_ratio, num_msrmnts, gas_temp_avg_dgc)
epx_par_table$SD <- c(flow_n2_sd, flow_co2_sd, gamma_sd, delta_sd, plasma_power_sd, sei_sd, yield_ratio_sd, 0, gas_temp_sd_dgc)
epx_par_table$RSD <- c(flow_n2_rsd*100, flow_co2_rsd*100, gamma_rsd*100, delta_rsd*100, plasma_power_rsd*100, sei_rsd*100, yield_ratio_rsd*100, 0, gas_temp_rsd * 100)
