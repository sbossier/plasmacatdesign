...
setwd("H:/data/co2-splitting/uhasselt/GM10.4")
filename <- "27.5s-01.csv"
...
subtitle: "Packing: `r unique(data$packing)`, residence time: `r unique(data$res_time_sec)`s & plasma power: `r data %>% filter(state == "plasma") %>% select(plasma_power_watt_avg) %>% unique() %>% round(digits = 2)`W"
...
kable(balance_c_table, digits = 3)
```
...
kable(balance_o_table, digits = 3)
```
