...
source('N:/FWET/FDCH/AdsCatal/General/Personal work folders/plasmacatdesign/scripts-R/dry-reforming-methane/dry-reforming-methane-functions.R')

# Read in starting data & replace common NA strings with a logical NA
setwd("N:/FWET/FDCH/AdsCatal/General/Personal work folders/vermile-valeria/T-REX/sasol-1.8_mno2-10%")
filename <- "drm_sasol-1.8_mno2-10%_01_01-corr.csv"
...
data           <- mutate(data,
                         conv_total     = ifelse(state == "plasma", conv_total, NA),
                         conv_total_sd  = ifelse(state == "plasma", conv_total_sd, NA),
                         conv_total_rsd = ifelse(state == "plasma", conv_total_rsd, NA),
                         .after = conv_rsd)
...
ee_gf_perc <- (alpha *
               ...
               -
                 sum(
                     unique(filter(data, state == "plasma" & compound %in% c("CH4", "CO2"))$conv) *
                     unique(filter(data, state == "blank"  & compound %in% c("CH4", "CO2"))$conc_avg_corr) *
                     unique(filter(data, state == "plasma" & compound %in% c("CH4", "CO2"))$gf)
                   ,
                   na.rm = T
                 )
) / sei * 100
  ...
  sei_rsd ^ 2 +
    (
      ...
      +
        sum(
            (
              unique(filter(data, state == "plasma" & compound %in% c("CH4", "CO2"))$conv_rsd ^ 2) +
              unique(filter(data, state == "blank"  & compound %in% c("CH4", "CO2"))$conc_avg_corr_rsd ^ 2)
            )
            *
              (
              unique(filter(data, state == "plasma" & compound %in% c("CH4", "CO2"))$conv) *
