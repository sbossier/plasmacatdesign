...
filename_selec <- "overview-selec-yield-balance-h.csv"
data_selec     <- read_csv(filename_selec, na = c("NA", "N/A", "N A", "Na", "na", "n a", "N a"))
data_selec     <- data_selec[order(data_selec$compound, data_selec$material),]
...
################################################
# Total Conversion vs. H2 Hydrogen Selectivity #
################################################
...
data %>% filter(compound == "H2") %>%
  ...
  geom_errorbar(aes(xmin = 100 * (conv - qt(.975, df_conv) * conv_sd / sqrt(df_conv + 1)),
                ...
                alpha = 0.5,
                show.legend = F) +
  geom_errorbar(aes(ymin = 100 * (selec_h - qt(.975, df) * selec_h_sd / sqrt(df + 1)),
                    ymax = 100 * (selec_h + qt(.975, df) * selec_h_sd / sqrt(df + 1))),
                width = 1,
                size  = 0.9,
                alpha = 0.5,
                show.legend = F) +
...
##################################################
# Total Conversion vs. C2H6 Hydrogen Selectivity #
##################################################
...
##################################################
# Total Conversion vs. C2H4 Hydrogen Selectivity #
##################################################
...
##################################################
# Total Conversion vs. C2H2 Hydrogen Selectivity #
##################################################
...
##################################################
# Total Conversion vs. C3H8 Hydrogen Selectivity #
##################################################
...
#####################################################
# Total Conversion vs. CH3OCH3 Hydrogen Selectivity #
#####################################################
...
data %>% filter(compound == "CH3OCH3") %>%
  ...
  scale_y_continuous(limits = c(0, 15),
                     expand = expansion(mult = c(0.0, 0.0))) +
  labs(x     = "Total Conversion (%)",
       y     = "Hydrogen Selectivity CH3OCH3 (%)",
       color = "Materials:") +
  theme_bw(base_size = 16) +
  scale_colour_colorblind() +
  theme(legend.position = "bottom",
...
###################################################
# Total Conversion vs. CH3OH Hydrogen Selectivity #
###################################################
...
####################################################
# Total Conversion vs. C2H5OH Hydrogen Selectivity #
####################################################
