...
setwd("H:/data/dry-reforming-methane/ugent/sasol-1.8")
...
data_total <- pivot_wider(data, id_cols = c(res_time, df), names_from = compound, values_from = c(mole_fraction, mole_fraction_sd)) %>%
  mutate(mole_fraction_total    = mole_fraction_CH4 + mole_fraction_CO2,
         mole_fraction_total_sd = sqrt(mole_fraction_sd_CH4 ^ 2 + mole_fraction_sd_CO2 ^ 2))

fit_total <- data_total %>%
  nls(mole_fraction_total ~ mole_fraction_total_eq - (mole_fraction_total_eq - 1) * exp(-k * res_time), data = ., #weights = weights_value,
      start = list(mole_fraction_total_eq = 0.50, k = 0.05))

fit_conv_total <- data %>%
  filter(compound == "CO2") %>%
  nls(conv_total ~ conv_total_eq - conv_total_eq * exp(-k * res_time), data = ., start = list(conv_total_eq = 0.50, k =0.05))
...
conv_total_eq     <- k_loss / k
conv_total_eq_rsd <- sqrt(k_loss_rsd ^ 2 + k_rsd ^ 2)
conv_total_eq_sd  <- conv_total_eq_rsd * conv_total_eq

#######################################################
# constructing the 95% prediction interval of the fit #
#######################################################

# fit_total_pred <- data.frame(res_time = seq(0, 100, .5))
# pred_values <- predictNLS(fit_total, newdata = fit_total_pred, interval = "prediction", alpha = .05, do.sim = FALSE)

# fit_total_pred$mean <- pred_values$summary[,2]
# fit_total_pred$lcl <- pred_values$summary[,5]
# fit_total_pred$ucl <- pred_values$summary[,6]
...
fit_total_conf <- data.frame(res_time = seq(1, 100, .5))
...
conf_values <- predictNLS(fit_total, newdata = fit_total_conf, interval = "confidence", alpha = .05, do.sim = FALSE)
...
# Total conversion in percent as fitted to model
fit_total_conf$mole_fraction_total_fit <- conf_values$summary[,2]

# SD of total conversion conversion in percent as fitted to model
...
# Lower 95% Confidence Level of total conversion in percent as fitted to model
fit_total_conf$mole_fraction_total_fit_lcl <- conf_values$summary[,5]

# Upper 95% Confidence Level of total conversion in percent as fitted to model
...
# Create plot of fitted and measured total conversion with 95% confidence interval for the standard deviation on the mean for the measured CH4 conversion
data_total %>%
  ggplot(aes(x = res_time,
             y = mole_fraction_total * 100)) +
  ...
  geom_ribbon(data  = fit_total_conf, 
              ...
              size  = 1.25) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = 100 * (mole_fraction_total - qt(0.975, df = df) * mole_fraction_total_sd / sqrt(df + 1)),
                    ymax = 100 * (mole_fraction_total + qt(0.975, df = df) * mole_fraction_total_sd / sqrt(df + 1))),
                width    = 1,
...
write_csv(fit_total_conf, gsub(".csv", "-total-fitting.csv", filename))